from comunidade_outpost import app, database, bcrypt
from comunidade_outpost.forms import FormLogin, FormCriarConta, FormEditarPerfil, FormCriarPost, FormEditarPost
from flask import render_template, request, flash, redirect, url_for, abort
from comunidade_outpost.models import Usuario, Post
...
def atualiza_cursos(form):
    lista_cursos = []
    for campo in form:
        if 'hobbie_' in campo.name:
            if campo.data:
...
def editar_perfil():
    ...
    if form.validate_on_submit():
        ...
        current_user.cursos = atualiza_cursos(form)
        database.session.commit()
        flash('Conta alterada.', 'alert-success')
